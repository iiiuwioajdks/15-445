 ...
 public:
  ...
  T GetElement(int i, int j) const override {
    if (i < 0 || i >= GetRowCount()) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "i out of range");
    }
    if (j < 0 || j >= GetColumnCount()) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "j out of range");
    }
    return data_[i][j];
  }
  ...
  void FillFrom(const std::vector<T> &source) override {
    ...
    if (size != GetColumnCount() * GetRowCount()) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "size out of range");
    }
...
class RowMatrixOperations {
 public:
  ...
  static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    int mul = matrixA->GetColumnCount();
    std::unique_ptr<RowMatrix<T>> res(new RowMatrix<T>(rows, cols));
    for (int i = 0; i < rows; ++i) {
      for (int j = 0; j < cols; ++j) {
        int sum = 0;
